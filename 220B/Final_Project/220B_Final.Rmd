...
summary(fit3.4 <- step(fit3.3,trace = F, direction = 'both', k = log(n)))
...
```{r}
summary(fitnb.2 <- step(fitnb, trace = F, direction='both'))
```
...
+ Need to check if categorical variables are significant, anova comparison between model w/ vs w/o
  fp is significant
  ch1 is not significant, can drop term

```{r}
summary(fitnb.3.2 <- update(fitnb.3, .~.-ch1))
```

```{r}
new.models <- list(fit3.3,fit3.4, fitnb.3,fitnb.3.2)
...
new.models.df <- data.frame("Model_Num" = c(max(model.performance.sumr$Model_Num)+1:length(new.models)),
                            "Type" = c('poisson','poisson','nB','nB'),
                            "Model_Name" = c("fit3.3","fit3.4","fitnb.3","fitnb.3.2"),
...
mu.nb <- exp(coef(fitnb.3.2)[1] + coef(fitnb.3.2)[2]*X[,'sex'] + coef(fitnb.3.2)[3]*X[,'fp']+
               coef(fitnb.3.2)[4]*X[,'ill'] + coef(fitnb.3.2)[5]*X[,'ad']+ coef(fitnb.3.2)[6]*X[,'hs'])
...
```{r NB simulation}
set.seed(123)
y.nb <- rnegbin(n,mu.nb,theta = fitnb.3.2$theta)

summary( sim.nb <- glm.nb(y.nb ~ ., data = X[,names(fitnb.3.2$model)[-1]]))
...
nb.pred <- predict.glm(fitnb.3.2, pharmacy_data[1,-1], type = 'response', se.fit = T)
...
prob.nb <- dnbinom(possiblities, size = fitnb.3.2$theta, mu = nb.pred$fit)
