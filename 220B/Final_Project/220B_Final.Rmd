...
summary(fit3.4 <- step(fit3.3,trace = F, direction = 'both'))
...
```{r}
termplot(fit3.3, data = pharmacy_data, partial.resid = T, col.res = 'black', cex = 1, pch = 1)
abline(h=0,lty=2, col='gray', lwd = 2)
```

```{r}
fv <- fitted(fit3.3)
plot(fv, residuals(fit3.3, type = 'response'), xlab = expression(log(hat(mu))),
     ylab = expression(log(y - hat(mu))^2))
lines(smooth.spline(fv,residuals(fit3.3, type = 'response')), col = 'red', lwd = 2)
```
...
```{r}
new.models <- list(fit3.3,fitnb.3,fitnb.3.2)
...
new.models.df <- data.frame("Model_Num" = c(max(model.performance.sumr$Model_Num)+1:length(new.models)),
                            "Type" = c('poisson','nB','nB'),
                            "Model_Name" = c("fit3.3","fitnb.3","fitnb.3.2"),
...
de.factorDF <- pharmacy_data %>% mutate_if(is.factor,as.numeric)

de.factorDF <- de.factorDF[-c(9,292,22),]

X <- as.data.frame(apply(de.factorDF,2,function(x) sample(min(x):max(x),n, replace = T)))

betas <- coef(fit3.3)[-1]

mu <- exp(coef(fit3.3)[1] + rowSums(sweep(X[,names(fit3.3$model)[-1]], MARGIN = 2, betas, '*')))
...
summary(glm(y ~ ., data = X[,names(fit3.3$model)[-1]], family = poisson(link=log)))
```
